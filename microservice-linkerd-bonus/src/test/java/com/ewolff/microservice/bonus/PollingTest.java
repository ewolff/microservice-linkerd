...
public class PollingTest {
	...
	private DslPart feedBody(Date now) {
		return new PactDslJsonBody().date("updated", "yyyy-MM-dd'T'kk:mm:ss.SSS+00:00", now)
									.eachLike("orders")
									.numberType("id", 1)
									.stringType("link", "http://localhost:8081/order/order/1")
									.date("updated", "yyyy-MM-dd'T'kk:mm:ss.SSS+00:00", now)
									.closeArray();
	}
